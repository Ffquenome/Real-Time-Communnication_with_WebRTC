...
function start() {
    ...
    */
    ...
    navigator.getUserMedia({audio:true, video:true}, successCallback, // leo: aparentemente tudo certo com a função start
...
function call() { 
    ...
    //if (navigator.webkitGetUserMedia) {
        ...
        if (localStream.getVideoTracks().length > 0) {
            log('Using video device: ' + localStream.getVideoTracks()[0].label); // leo:coloquei trace ao inves de log
        }
        if (localStream.getAudioTracks().length > 0) {
            log('Using audio device: ' + localStream.getAudioTracks()[0].label);// leo:coloquei trace ao inves de log
        }
    //}
...
function gotRemoteStream(event){
    remoteVideo.srcObject = event.stream; ///////////////////////// modifiqued
